...
function renderRookies(){var out="";for(var i=0;i<DATA.rookies.length;i++){var r=DATA.rookies[i];var d=r.draftYear+" • Rd "+r.draftRound+", pick "+r.pickInRound+" in round • #"+r.overallPick+" overall";out+='<div class="player"><span class="rank">#'+r.rank+'</span><span class="pname"><b>'+esc(r.name)+'</b><small>'+esc(r.team)+' • '+esc(d)+'</small><small>'+esc(r.rationale||"")+'</small></span><span>'+r.pos+'</span><span></span></div>'}byId("rlist").innerHTML=out}
