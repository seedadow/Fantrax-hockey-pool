...
var DATA=window.TRUE_NORTH_DATA, KEY="trueNorthDraftV4Fantrax";
...
function teamLive(t){var base=DATA.teams[t],spent=0,n=0;for(var i=0;i<state.drafted.length;i++){if(state.drafted[i].owner===t){spent+=Number(state.drafted[i].price);n++}}return{cap:Number(base.cap)-spent,open:Math.max(0,Number(base.open)-n)}}
...
function renderPlayers(){
 ...
 out+='<button class="player" type="button" data-player="'+esc(p.name)+'"><span class="rank">#'+p.tnRank+'</span><span class="pname"><b>'+esc(p.name)+'</b><small>'+p.pos+' · '+esc(p.team)+' · GP '+(p.gp==null?"—":p.gp)+inj+'</small></span><span class="money">'+money(p.tnValue)+'<small>TN</small></span><span class="money">'+money(p.maxBid)+'<small>MAX</small></span></button>'}
...
function show(name){var p=null;for(var i=0;i<DATA.players.length;i++)if(DATA.players[i].name===name){p=DATA.players[i];break}if(!p)return;selected=p;var sold=drafted(name),inj=(p.injury&&p.injury.indexOf("CLEAR")<0)?'<p class="inj"><b>'+esc(p.injury)+'</b><br>'+esc(p.return||"No firm return date")+'</p>':'<p class="good">No material current injury adjustment</p>';
 ...
 var body='<div class="big">'+esc(p.name)+'</div><div class="muted">'+p.pos+' · '+esc(p.team)+' · GP '+p.gp+'</div><div class="kv"><div><small>TN Rank</small><b>#'+p.tnRank+'</b></div><div><small>Pool Rank</small><b>#'+p.poolRank+'</b></div><div><small>Market Est.</small><b>'+money(p.marketPrice)+'</b></div><div><small>TN Value</small><b>'+money(p.tnValue)+'</b></div><div><small>Maximum Bid</small><b>'+money(p.maxBid)+'</b></div></div><div class="muted">'+stats+'</div>'+inj;
...
function renderTeams(){var teams=Object.keys(DATA.teams),out="";for(var i=0;i<teams.length;i++){var t=teams[i],b=DATA.teams[t],x=teamLive(t),rs=DATA.rosters[t]||[];out+='<div class="team"><div class="teamhead"><b>'+esc(t)+'</b><span class="muted">'+b.regular+' salary players · '+b.min+' MIN · '+b.picks+' rookie picks</span></div><div class="kv"><div><small>Money left</small><b>'+money(x.cap)+'</b></div><div><small>Open spots</small><b>'+x.open+'</b></div><div><small>Fantrax salaries</small><b>'+money(b.fantraxSalary)+'</b></div><div><small>Forced promotions</small><b>'+b.forcedPromotions+' ('+money(b.promotionCost)+')</b></div></div><div class="roster">';for(var j=0;j<rs.length;j++){var r=rs[j];out+='<div class="rrow"><span>'+esc(r.name)+' <small>'+esc(r.pos)+' · '+esc(r.nhlTeam)+(r.status==="MIN"?' · MIN':'')+'</small></span><b>'+money(r.salary)+'</b></div>'}out+='</div></div>'}byId("teamgrid").innerHTML=out}
function renderRookies(){var out="";for(var i=0;i<DATA.rookies.length;i++){var r=DATA.rookies[i];var d=r.draftYear+" • Rd "+r.draftRound+(r.pickInRound!=null?", pick "+r.pickInRound+" in round":"")+" • #"+r.overallPick+" overall";out+='<div class="player"><span class="rank">#'+r.rank+'</span><span class="pname"><b>'+esc(r.name)+'</b><small>'+esc(r.team)+' • '+esc(d)+'</small><small>'+esc(r.rationale||"")+'</small></span><span>'+r.pos+'</span><span></span></div>'}byId("rlist").innerHTML=out}
