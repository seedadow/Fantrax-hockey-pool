...
function show(name){var p=null;for(var i=0;i<DATA.players.length;i++)if(DATA.players[i].name===name){p=DATA.players[i];break}if(!p)return;selected=p;var sold=drafted(name),inj=(p.injury&&p.injury.indexOf("CLEAR")<0)?'<p class="inj"><b>'+esc(p.injury)+'</b><br>'+esc(p.return||"No firm return date")+'</p>':'<p class="good">No material current injury adjustment</p>';
 ...
 var body='<div class="big">'+esc(p.name)+'</div><div class="muted">'+p.pos+' · '+esc(p.team)+' · GP '+p.gp+'</div><div class="kv"><div><small>TN Rank</small><b>#'+p.tnRank+'</b></div><div><small>Pool Rank</small><b>#'+p.poolRank+'</b></div><div><small>TN Value</small><b>'+money(p.tnValue)+'</b></div><div><small>Maximum Bid</small><b>'+money(p.maxBid)+'</b></div></div><div class="muted">'+stats+'</div>'+inj;
...
function renderRookies(){var out="";for(var i=0;i<DATA.rookies.length;i++){var r=DATA.rookies[i];out+='<div class="player"><span class="rank">#'+r.rank+'</span><span class="pname"><b>'+esc(r.name)+'</b><small>'+esc(r.team)+'</small></span><span>'+r.pos+'</span><span></span></div>'}byId("rlist").innerHTML=out}
